...
export class ClassificationImageService {
  ...
  constructor(
    @Inject('MODEL_PATH') private readonly modelPath: string,
  ...
  async classifyImage(
    ...
    try {
      const flagClassifyImage = await this.featureFlagService.get(
        'VALIDATE_IMAGE_PET_IS_DOG_OR_CAT',
      );

      if (!flagClassifyImage.isEnabled) {
        this.logger.warn(`Flag classify image is not enabled`);
        return null;
      }
